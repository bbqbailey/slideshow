...
def extract_weather_summary_from_onecall(data):
    ...
    if sunrise is None and daily and "sunrise" in daily[0]:
        sunrise = daily[0]["sunrise"]
    if sunset is None and daily and "sunset" in daily[0]:
    ...
    if len(daily) >= 1:
        today = daily[0]
        try:
            out["hi_today"] = round(today["temp"]["max"])
            out["lo_today"] = round(today["temp"]["min"])
        except Exception:
            pass
    ...
    if len(daily) >= 2:
        tomorrow = daily[1]
        try:
            out["hi_tomorrow"] = round(tomorrow["temp"]["max"])
            out["lo_tomorrow"] = round(tomorrow["temp"]["min"])
        except Exception:
            pass
...
""")
    ...
    if os.path.exists(OUTPUT_PNG):
        ...
        lines.append("<p><img src='/weather_panel.png' alt='Weather panel image'></p>")
    else:
    ...
    if not minutely:
        ...
        # Show up to first 20 entries to keep page manageable
        lines.append("<table>")
    ...
    if not hourly:
        ...
        for entry in hourly[:24]:
            ...
            pop = entry.get("pop", 0)  # probability of precip (0-1)
            pop_pct = int(pop * 100)
            rain_mm = 0
            rain = entry.get("rain")
...
class WeatherRequestHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt, *args):
        # Route HTTP server logs into the main logger instead of stderr
