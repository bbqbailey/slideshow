#!/usr/bin/env python3
# Slideshow.py - Version V3-0
#
# Base: V1-0 (MJPEG push, recursive directory walking)
#
# V2-0 additions:
...
#
# V2-1 additions:
# - Adds a WEATHER frame using weather_panel.png generated by weather_panel.py
# - New cycle:
#       photos -> TIME -> WEATHER -> RADAR 1 -> RADAR 2 -> repeat
#
# V2-2 additions:
# - After RADAR 2, show 6 security camera frames (latest.jpg from each camera subdir)
#   Order:
#       backwindows, basementWoodRm, driveway, frontDoorInside, garagedoors, livingRoom
#
# V3-0 additions (major semantic change):
# - Removes all randomness (no random directory choice, no shuffle).
# - Deterministic traversal:
#       starting roots in file order (slideshowDirectories.txt or preferred list)
#       depth-first walk through each root and its subdirectories
#       show image files in whatever filesystem order is returned (no sorting)
# - No per-directory cap (shows ALL image files encountered).
# - Continues on per-image error (placeholder slide) without skipping directory.
# - No transition frame between directories/roots.
#
# Current config (verify these match what you intend):
# - SLIDE_DURATION_SECONDS = 10
# - IMAGE_COUNT_DISPLAY    = 30
# - Non-photo insert block after every IMAGE_COUNT_DISPLAY photos:
#       TIME -> WEATHER -> RADAR1 -> RADAR2 -> SECURITY x6
...
# NOTE: IMAGES_PER_DIRECTORY is deprecated in V3-0 (no per-directory cap).
IMAGES_PER_DIRECTORY = 75

SLIDE_DURATION_SECONDS = 10          # per-frame display time (photos + non-photos)
IMAGE_COUNT_DISPLAY = 30            # number of photo images before TIME + WEATHER + RADAR1 + RADAR2 (+ SECURITY)
...
display_logger.info("=== DISPLAY LOG STARTED (V3-0 MJPEG + RADAR + WEATHER + SECURITY, deterministic DFS) ===")
...
# ======================================================================
#  (Legacy) Recursive Directory Walker (kept for reference; not used in V3-0)
# ======================================================================
def collect_image_directories(starting_dirs):
    """
    Legacy helper from V1/V2: collects directories that contain at least one image file.
    V3-0 does NOT use this; it streams a deterministic depth-first walk instead.
    """
    collected = []
...
def image_path_to_jpeg_bytes(image_path: str) -> bytes:
    ...
    On error: return placeholder (and continue slideshow).
    """
    try:
...
def generate_weather_frame() -> bytes:
    """
    Load weather_panel.png (generated by weather_panel.py) and convert it to JPEG bytes.
...
def generate_security_frames_in_order():
    """
    Return a list of (jpeg_bytes, label) for the 6 security camera latest.jpg files, in order.
...
# ======================================================================
#  V3-0 DETERMINISTIC DEPTH-FIRST TRAVERSAL
# ======================================================================
def _walk_roots_depth_first_no_sort(starting_roots):
    """
    Generator yielding image file paths in deterministic *structure* order:
      - roots in the exact order provided by slideshowDirectories.txt / preferred list
      - depth-first traversal under each root
      - NO sorting of dirs/files (accept whatever order os.walk provides)

    Skips any directory named exactly 'slideshow_exclude' (and prunes its subtree).
    """
    def onerror(err):
    ...
    for root0 in starting_roots:
        ...
        for root, dirs, files in os.walk(root0, topdown=True, onerror=onerror, followlinks=False):
            # Prune any child dirs named exactly slideshow_exclude
            dirs[:] = [d for d in dirs if d != 'slideshow_exclude']

            # If the current directory itself is slideshow_exclude, skip it (belt + suspenders)
            if os.path.basename(root) == 'slideshow_exclude':
                continue

            # Yield image files in the order returned by os.walk (no sorting)
            for fname in files:
                try:
...
def generate_slideshow_images(starting_roots, time_interval):
    """
    V3-0 core producer loop (no randomness).

    Traversal:
      - Starting roots (in file order) -> depth-first walk -> image files in os.walk order (no sorting)

    Sequence:
      - Show IMAGE_COUNT_DISPLAY photo slides
      - Then show: TIME -> WEATHER -> RADAR 1 -> RADAR 2 -> SECURITY x6
      - Repeat that insert block every IMAGE_COUNT_DISPLAY photos, continuing traversal where it left off.

    When traversal completes:
      - Wrap back to the beginning and continue (in the then-current filesystem order).
    """
    ...
    while True:
        ...
        for image_path in _walk_roots_depth_first_no_sort(starting_roots):
            ...
            # Show a photo (placeholder on error, continue)
            update_current_frame(image_path_to_jpeg_bytes(image_path), image_path)
            time.sleep(duration)

            photo_counter += 1

            # After IMAGE_COUNT_DISPLAY photos, inject TIME + WEATHER + RADAR1 + RADAR2 + SECURITY
        ...
        if not any_photo_shown:
            ...
            # Completed full traversal; wrap to start with no transition frame.
            logging.info("Completed full traversal. Wrapping to start (V3-0).")
...
class SlideshowHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def get_html_content(self):
        ...
        <head>
            <meta charset="UTF-8">
            <title>Slideshow V3-0 MJPEG + RADAR + WEATHER + SECURITY (Deterministic DFS)</title>
            <style>
                html, body {
...
def run_server(port=8000):
    ...
    logging.info("Server on port %s (V3-0 MJPEG + RADAR + WEATHER + SECURITY, deterministic DFS, threaded)", port)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Slideshow V3-0 — Deterministic depth-first photo viewer with MJPEG push stream, weather panel, radar.weather.gov ridge frames, and security camera frames",
