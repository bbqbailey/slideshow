#!/usr/bin/env python3
# Slideshow.py - Version 0-4
# Changes in V0-4:
# - slideshowDirectories.txt now lists STARTING DIRECTORIES
# - Recursively walks all subdirectories
# - Skips any directory whose basename is exactly "slideshow_exclude"
# - Collects every directory that contains at least one image file
# - Directory handling fully dynamic
...
#  - skips any directory whose basename == "slideshow_exclude"
# ======================================================================
...
class SlideshowHTTPRequestHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/slideshow':
            ...
            self.end_headers()
            with open('/tmp/resized-image.jpg', 'rb') as f:
...
def generate_time_image():
    ...
    img.save('/tmp/resized-image.jpg')
...
def serve_image(image_path):
    try:
        ...
        img = img.convert("RGB")
        img.save('/tmp/resized-image.jpg', 'JPEG', quality=95)
        ...
        draw.text((50, 50), f"SKIPPED: {os.path.basename(image_path)}",
                  fill=(255, 255, 255))
        placeholder.save('/tmp/resized-image.jpg')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Slideshow V4-0 — Recursive Photo Viewer",
