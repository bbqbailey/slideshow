#!/usr/bin/env python3
# Slideshow-V2-0.py - Version V2-0
#
...
# - Sequence (per cycle of 15 frames):
#       12 photo images  ->  TIME  ->  RADAR1  ->  RADAR2
...
IMAGE_COUNT_DISPLAY = 12            # number of photo images before TIME + RADAR1 + RADAR2
...
display_logger.info("=== DISPLAY LOG STARTED (V2-0 MJPEG + RADAR) ===")
...
def generate_slideshow_images(directories, time_interval):
    ...
    Sequence per cycle:
      ...
      - Then show: TIME -> RADAR 1 -> RADAR 2
      - Repeat.
    """
    duration = time_interval
    photo_counter = 0  # counts photos within the current 12-photo block
    ...
    while True:
        ...
        for image_path in image_files:
            ...
            # After IMAGE_COUNT_DISPLAY photos, inject TIME + RADAR1 + RADAR2
...
class SlideshowHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def get_html_content(self):
        ...
        <head>
            <meta charset="UTF-8">
            <title>Slideshow V2-0 MJPEG + RADAR</title>
            <style>
                html, body {
...
def run_server(port=8000):
    ...
    logging.info(f"Server on port {port} (V2-0 MJPEG + RADAR, threaded)")
    logging.info("Serving on http://0.0.0.0:8000/slideshow")
    httpd.serve_forever()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Slideshow V2-0 — Recursive Photo Viewer with MJPEG push stream and radar.weather.gov ridge frames",
